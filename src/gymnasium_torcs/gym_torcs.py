...
import gymnasium_torcs.snakeoil3_gym as snakeoil3
import numpy as np
import copy
import os
import time
...
class TorcsEnv (Env):
    ...
    def __init__(self, vision=False, throttle=False, gear_change=False, render_mode = None):
        ...
        os.system('pkill '+ TorcsEnv.torcs_exec_path)
        time.sleep(0.5)
        if self.vision is True:
            os.system(TorcsEnv.torcs_exec_path + ' -nofuel -nodamage -nolaptime  -vision &')
        else:
            os.system(TorcsEnv.torcs_exec_path + ' -nofuel -nodamage -nolaptime &')
        time.sleep(0.5)
        autostart_path = os.path.join(TorcsEnv.gym_torcs_path, "autostart.sh")
